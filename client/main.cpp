...
void switchLight()
{
    ...
    if (objectNode) {
        ...
        // Salviamo Diffuse e Specular perch� sono quelle che illuminano la scena
        ...
        if (spotlight) {
            ...
            // Salva lo stato originale solo la prima volta (quando � sicuramente accesa)
            ...
            std::cerr << "[Error] L'oggetto 'Spot001' non � una SpotLight." << std::endl;
        }
    }
    else {
...
int main() {
    ...
    if (ovoScene) {
        ...
        //Viene passata le scena perch� i dischi sono i Disk_n sono dentro il .ovo
        HanoiGame::init(scene);
        HanoiGame::showHelp();
    }
    else {
